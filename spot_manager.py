...
async def main():
    ...
    await page.waitForSelector('body > table > tbody > tr:nth-child(2) > td > table > tbody > tr > td > form > table > tbody > tr:nth-child(2) > td:nth-child(2) > input[type=text]')
    await page.waitForSelector('body > table > tbody > tr:nth-child(2) > td > table > tbody > tr > td > form > table > tbody > tr:nth-child(3) > td:nth-child(2) > input[type=password]')
    ...
    await username.type('mxplan.cortegab')    #se rellena
    await password.type('Casabrave6')
    ...
    #print(popI)
    popU=browser.targets()
    #print(popU)
    # await page.waitFor("[id='ShipmentFinancials']")
    # await page.click("[id='ShipmentFinancials']")
    pop = browser.targets()[4]
    await page.waitFor(3000)
    popup = await pop.page()
    #print(popup.url)
    frame = popup.frames[2]
    #print(frame.url)
    await frame.waitForSelector("[id='tabShipmentFinancials']")
    await frame.click("[id='tabShipmentFinancials']")
    await frame.waitForSelector('#\\31 3502054 > td.gridBodyCell.gridBodyBtnsCell > table > tbody > tr > td:nth-child(1) > a > img')
    await frame.click('#\\31 3502054 > td.gridBodyCell.gridBodyBtnsCell > table > tbody > tr > td:nth-child(1) > a > img')
    await frame.waitFor(5500)
    costo = await frame.J('#shipment_cost\\/cost\\@PRF')

    await costo.type('243524')
